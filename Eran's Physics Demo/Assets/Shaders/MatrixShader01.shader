Shader "Custom/Matrix"
{
	Properties
	{
		...
		_TextColor("Text Color", Color) = (0,1,0,1)
		...
		SubShader
		{
			...
			fixed4 frag(v2f_img i) : SV_Target
			{
				...
				float4 out1 = 1.0 * _TextColor * val;
				...
				dissolve_value += float4(1, 1, 1, 1) * step(dissolve_value - _Amount, 0.1f); //white color with 0.05 border size

				return out1 * float4(1, 1, 1, dissolve_value);
			}
			ENDCG
		}
	}
